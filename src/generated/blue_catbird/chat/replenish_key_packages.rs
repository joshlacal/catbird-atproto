...
//
// Lexicon: blue.catbird.chat.replenishKeyPackages
//
...
impl core::fmt::Display for ReplenishKeyPackagesError {
...
/** Response marker for the `blue.catbird.chat.replenishKeyPackages` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `ReplenishKeyPackagesOutput<S>` for this endpoint.*/
pub struct ReplenishKeyPackagesResponse;
impl jacquard_common::xrpc::XrpcResp for ReplenishKeyPackagesResponse {
    const NSID: &'static str = "blue.catbird.chat.replenishKeyPackages";
    const ENCODING: &'static str = "application/json";
    type Output<S: jacquard_common::BosStr> = ReplenishKeyPackagesOutput<S>;
...
/** Endpoint marker for the `blue.catbird.chat.replenishKeyPackages` procedure.

Path: `/xrpc/blue.catbird.chat.replenishKeyPackages`. The request payload type is `ReplenishKeyPackages<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
pub struct ReplenishKeyPackagesBuilder<
    St: replenish_key_packages_state::State,
...
impl<St, S: jacquard_common::BosStr> ReplenishKeyPackagesBuilder<St, S>
where
    St: replenish_key_packages_state::State,
    St::SignedRequest: replenish_key_packages_state::IsUnset,
{
...
where
    St: replenish_key_packages_state::State,
    St::SignedRequest: replenish_key_packages_state::IsSet,
{
    /// Build the final struct.
    pub fn build(self) -> ReplenishKeyPackages<S> {
