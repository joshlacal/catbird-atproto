...
//
// Lexicon: blue.catbird.chat.acknowledgeWelcome
//
...
impl core::fmt::Display for AcknowledgeWelcomeError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::AcknowledgementConflict(msg) => {
                write!(f, "AcknowledgementConflict")?;
                if let Some(msg) = msg {
                    write!(f, ": {}", msg)?;
                }
                Ok(())
...
/** Response marker for the `blue.catbird.chat.acknowledgeWelcome` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `AcknowledgeWelcomeOutput<S>` for this endpoint.*/
pub struct AcknowledgeWelcomeResponse;
impl jacquard_common::xrpc::XrpcResp for AcknowledgeWelcomeResponse {
    const NSID: &'static str = "blue.catbird.chat.acknowledgeWelcome";
    const ENCODING: &'static str = "application/json";
    type Output<S: jacquard_common::BosStr> = AcknowledgeWelcomeOutput<S>;
...
/** Endpoint marker for the `blue.catbird.chat.acknowledgeWelcome` procedure.

Path: `/xrpc/blue.catbird.chat.acknowledgeWelcome`. The request payload type is `AcknowledgeWelcome<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
pub struct AcknowledgeWelcomeBuilder<
    St: acknowledge_welcome_state::State,
...
impl<S: jacquard_common::BosStr> AcknowledgeWelcome<S> {
    /// Create a new builder for this type
    pub fn builder() -> AcknowledgeWelcomeBuilder<acknowledge_welcome_state::Empty, S> {
        AcknowledgeWelcomeBuilder::builder()
    }
}
...
impl<St, S: jacquard_common::BosStr> AcknowledgeWelcomeBuilder<St, S>
where
    St: acknowledge_welcome_state::State,
    St::SignedRequest: acknowledge_welcome_state::IsUnset,
{
...
where
    St: acknowledge_welcome_state::State,
    St::SignedRequest: acknowledge_welcome_state::IsSet,
{
    /// Build the final struct.
    pub fn build(self) -> AcknowledgeWelcome<S> {
        AcknowledgeWelcome {
