...
fn legacy_output_decodes_without_receipt() {
    let output: CommitGroupChangeOutput =
...
fn full_receipt_decodes_and_round_trips_losslessly() {
    ...
    let output: CommitGroupChangeOutput = serde_json::from_str(fixture).unwrap();
    let receipt = output.receipt.as_ref().unwrap();
    assert_eq!(receipt.convo_id.as_str(), "convo-1");
    ...
    let round_trip: CommitGroupChangeOutput = serde_json::from_str(&encoded).unwrap();
...
fn malformed_present_receipts_are_rejected() {
    ...
    for fixture in malformed {
        assert!(
            serde_json::from_str::<CommitGroupChangeOutput>(fixture).is_err(),
